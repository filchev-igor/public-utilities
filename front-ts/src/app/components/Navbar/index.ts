...
const Navbar = async () => {
  ...
  setTimeout(() => {
    ...
    if (lithuanianButton) { lithuanianButton.onclick = setIsLithuanian(); }

    if (englishButton) { englishButton.onclick = setIsLithuanian(false); }

    if (lightModeButton) {
      lightModeButton.addEventListener('click', setHasDayMode());
    }

    if (darkModeButton) { darkModeButton.addEventListener('click', setHasDayMode(false)); }
